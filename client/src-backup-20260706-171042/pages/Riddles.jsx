...
import { 
  Puzzle, Brain, Lightbulb, Sparkles, 
  Trophy, Clock, Zap, Check, X,
  Send, Eye, EyeOff, RefreshCw,
  Star, Medal, Award, Users, Loader2,
...
const Riddles = () => {
  ...
  const toggleRiddle = (riddleId) => {
    if (expandedRiddle === riddleId) {
      ...
      // Reset states for this riddle when opening
  ...
  const handleSubmitRiddle = (riddleId) => {
    ...
    if (!userAnswer) {
      toast.warning('Please enter your answer!', { position: 'top-right', autoClose: 1500 });
      return;
    }
    ...
    if (isCorrectAnswer) {
      toast.success(`🎉 Correct! +${riddle.points} points`, { position: 'top-right', autoClose: 1500 });
      ...
      toast.error(`❌ Not quite! Try again or see the answer.`, { position: 'top-right', autoClose: 1500 });
      setStreak(0);
  ...
  const handleKeyPress = (riddleId, e) => {
    if (e.key === 'Enter') {
        ...
        <div className="glass-card p-6 bg-gradient-to-r from-[#6C2BD9]/10 to-[#8B5CF6]/10 border border-[#6C2BD9]/20">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white font-medium mt-1">{currentRiddle.question}</p>
              </div>
            </div>
            <button
          ...
          <div className="flex flex-col md:flex-row gap-3 mt-2">
            <div className="flex-1">
              <div className="relative">
                <input
                  ...
                  onKeyPress={(e) => {
                    if (e.key === 'Enter') {
                      ...
                      // Handle daily riddle submission
                      const userAnswer = answer.trim().toLowerCase();
                      const correctAnswer = dailyRiddle.answer.toLowerCase();
                      const isCorrectAnswer = userAnswer === correctAnswer || 
                                             userAnswer.includes(correctAnswer) || 
                                             correctAnswer.includes(userAnswer);
                      ...
                      if (isCorrectAnswer) {
                        toast.success(`🎉 Correct! +${dailyRiddle.points} points`, { position: 'top-right', autoClose: 1500 });
                        setSolvedCount(prev => prev + 1);
                        setPoints(prev => prev + dailyRiddle.points);
                        setStreak(prev => prev + 1);
                      } else {
                        toast.error(`❌ Not quite right.`, { position: 'top-right', autoClose: 1500 });
                        setStreak(0);
                      }
                    }
                  }}
                ...
                >
                  {isCorrect === true ? <Check className="w-5 h-5" /> : <Send className="w-4 h-4" />}
                </button>
              </div>
            </div>
          ...
          {showHint && (
            <div className="mt-3 p-3 bg-yellow-400/10 border border-yellow-400/20 rounded-lg">
              <p className="text-yellow-400 text-sm">💡 {currentRiddle.hint}</p>
            </div>
          )}
          {showAnswer && (
            <div className="mt-3 p-3 bg-[#00C9A7]/10 border border-[#00C9A7]/20 rounded-lg">
              <p className="text-[#00C9A7] text-sm">
                <span className="font-semibold">Answer:</span> {currentRiddle.answer}
              </p>
              <p className="text-gray-400 text-xs mt-1">{currentRiddle.explanation}</p>
            </div>
          )}
        </div>
      )}

      {/* All Riddles - Expandable Cards */}
                    ...
                    >
                      <div className="border-t border-white/10 pt-4 space-y-3">
                        ...
                        <div className="flex flex-col md:flex-row gap-3">
                          <div className="flex-1">
                            <div className="relative">
                              <input
                                ...
                                onKeyPress={(e) => handleKeyPress(riddle.id, e)}
                                placeholder="Type your answer..."
                                className="w-full px-4 py-3 bg-[#2D2D5E] rounded-lg border border-white/10 text-white placeholder-gray-500 focus:border-[#6C2BD9] focus:outline-none focus:ring-2 focus:ring-[#6C2BD9]/20 transition-all"
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => setShowHintMap(prev => ({ ...prev, [riddle.id]: !prev[riddle.id] }))}
                            ...
                            <button
                              onClick={() => setShowAnswerMap(prev => ({ ...prev, [riddle.id]: !prev[riddle.id] }))}
                              className="btn-secondary text-sm px-3 py-2 flex items-center gap-1"
                            >
