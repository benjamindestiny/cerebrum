...
const Profile = () => {
  ...
  const { currentUser, userProfile, setUserProfile, updateProfile } = auth;
        ...
        if (quizHistory && quizHistory.length > 0) {
          ...
          for (const record of quizHistory) {
            ...
            if (!lastDate) {
              ...
              const daysDiff = Math.floor((lastDate - recordDate) / (1000 * 60 * 60 * 24));
              if (daysDiff === 1) {
                currentStreak++;
                lastDate = recordDate;
              } else if (daysDiff > 1) {
                    ...
                    <div className="mt-2 bg-[#2d2d44] rounded-lg p-3 shadow-xl">
                      <div className="space-y-2 max-h-40 overflow-y-auto">
                        {DEFAULT_BIOS.map((bioText, index) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded hover:bg-[#3d3d54] transition-colors ${
                              selectedBio === bioText ? 'bg-[#7c3aed]' : 'text-gray-300'
                            }`}
                          >
                            {bioText}
                          </button>
                        ...
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded hover:bg-[#3d3d54] transition-colors ${
                            selectedBio === 'custom' ? 'bg-[#7c3aed]' : 'text-gray-300'
                          }`}
                        >
                          ✏️ Write custom bio
                        </button>
