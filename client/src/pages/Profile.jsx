    ...
    try {
      ...
      if (profileData) {
        ...
        setFormData({
          ...
          email: user?.email || "",
          avatar_id: profileData.avatar_id || 1,
          bio: profileData.bio || "",
    ...
    <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 px-3 sm:px-4 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {editing ? (
            <>
              <button onClick={() => { setEditing(false); loadUserData(user.id); }} className="px-3 py-1.5 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-colors text-xs flex items-center gap-1.5" disabled={saving}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
        <div className="bg-[#1a1a2e] rounded-xl p-3 flex items-center gap-3 border border-white/5">
          ...
          <div><div className="text-lg font-bold text-white">{stats.riddlesSolved || 0}</div><div className="text-[10px] text-gray-400">Riddles Solved</div></div>
        </div>
        <div className="bg-[#1a1a2e] rounded-xl p-3 flex items-center gap-3 border border-white/5">
          <div className="p-2 bg-green-400/10 rounded-lg"><BookOpen className="w-5 h-5 text-green-400" /></div>
          <div><div className="text-lg font-bold text-white">{stats.readArticles || 0}</div><div className="text-[10px] text-gray-400">Articles Read</div></div>
        </div>
        <div className="bg-[#1a1a2e] rounded-xl p-3 flex items-center gap-3 border border-white/5">
          <div className="p-2 bg-purple-400/10 rounded-lg"><Star className="w-5 h-5 text-purple-400" /></div>
          <div><div className="text-lg font-bold text-white">{stats.perfectScores || 0}</div><div className="text-[10px] text-gray-400">Perfect Scores</div></div>
        </div>
      </div>
