...
import { Mail, Send, Users, Loader2, CheckCircle, XCircle, Calendar } from 'lucide-react';
...
const AdminWeeklyReport = () => {
  ...
  const sendWeeklyReport = async () => {
    ...
    if (!confirm('Send weekly report to ALL users?')) return;
    ...
    <div className="max-w-4xl mx-auto p-6  text-white border-[#2A2A4A]">
      <div className="glass-card p-6 border border-white/5  text-white border-[#2A2A4A]">
        <div className="flex items-center gap-3 mb-6  text-white border-[#2A2A4A]">
          <Calendar className="w-8 h-8 text-[#3B82F6CC]  text-white border-[#2A2A4A]" />
          <h1 className="text-2xl font-bold text-white  text-white border-[#2A2A4A]">Weekly Reports</h1>
        </div>
        ...
        <div className="bg-teal-400/10 border border-teal-400/20 rounded-lg p-4 mb-6  text-white border-[#2A2A4A]">
          <p className="text-teal-400 text-sm flex items-center gap-2  text-white border-[#2A2A4A]">
            ...
            This will send a weekly report to ALL users.
          </p>
          <p className="text-gray-400 text-xs mt-1  text-white border-[#2A2A4A]">
            Includes: quiz stats, average score, points, and recent activity.
            <br />Brevo free tier: 300 emails/day
          </p>
        </div>
        ...
        >
          ...
          {loading ? 'Sending...' : 'Send Weekly Report to All Users'}
        </button>

        {result && (
          <div className={`mt-4 p-4 rounded-lg border ${result.error ? 'border-red-500/30 bg-red-500/10' : 'border-green-500/30 bg-green-500/10'}`}>
            {result.error ? (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 text-sm  text-white border-[#2A2A4A]">
                  <div className="bg-white/5 p-2 rounded  text-white border-[#2A2A4A]">
                    ...
                    <p className="text-white font-bold  text-white border-[#2A2A4A]">{result.totalUsers || 0}</p>
                  </div>
                  <div className="bg-white/5 p-2 rounded  text-white border-[#2A2A4A]">
                    <p className="text-gray-400  text-white border-[#2A2A4A]">Sent</p>
                    <p className="text-green-400 font-bold  text-white border-[#2A2A4A]">{result.sentCount || 0}</p>
                  </div>
                </div>
                {result.errors?.length > 0 && (
                  <div className="mt-3  text-white border-[#2A2A4A]">
                    <p className="text-red-400 text-sm  text-white border-[#2A2A4A]">Failed to send to {result.errors.length} users</p>
                  </div>
                )}
              </div>
            )}
