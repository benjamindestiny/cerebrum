...
const Results = () => {
    ...
    return (
      <div className="flex items-center justify-center min-h-[300px] sm:min-h-[400px] px-4">
        <div className="text-center">
          <Loader2 className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-[#6C2BD9] animate-spin mx-auto mb-3 sm:mb-4" />
  ...
  const { score, correct, total, questions, answers, category, difficulty } =
    results;
  ...
  const getGrade = () => {
    ...
    if (score >= 50)
      return { label: "Good Effort! 💪", color: "text-[#6C2BD9]", emoji: "💪" };
    ...
    <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6 px-3 sm:px-4 pb-12">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-3 mt-3 text-[10px] sm:text-xs text-gray-500 flex-wrap">
          ...
          <span
            className={`px-1.5 sm:px-2 py-0.5 rounded-full ${getDifficultyColor()}`}
          >
            {difficulty || "Medium"}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 mt-4 sm:mt-6 justify-center">
          <button
            ...
            className="btn-primary text-xs sm:text-sm md:text-base px-3 sm:px-4 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2"
          >
          ...
          >
            {shareCopied ? (
              <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4 text-[#00C9A7]" />
            ) : (
