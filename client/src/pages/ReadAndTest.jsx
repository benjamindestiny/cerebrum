...
import { 
  BookOpen, Brain, Clock, Award, ArrowLeft,
  ChevronRight, Check, X, Loader2,
  Sparkles, TrendingUp, Users, Target,
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="glass-card px-4 py-2 flex items-center gap-2">
            ...
            <span className="text-sm text-gray-300">{materials.length} Articles</span>
          </div>
        </div>
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    material.difficulty === 'Easy' ? 'bg-green-500/20 text-green-400' :
                    material.difficulty === 'Medium' ? 'bg-yellow-500/20 text-yellow-400' :
  ...
  const renderReadingView = () => {
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <button onClick={handleGoBack} className="text-gray-400 hover:text-white transition-colors flex items-center gap-2 text-sm">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-[#6C2BD9]">{selectedMaterial.category}</span>
                <span className="text-xs text-gray-500">• {selectedMaterial.subcategory}</span>
              </div>
              <h2 className="text-2xl font-bold text-white">{selectedMaterial.title}</h2>
            </div>
            <span className={`text-xs px-2 py-1 rounded-full ${
              selectedMaterial.difficulty === 'Easy' ? 'bg-green-500/20 text-green-400' :
              selectedMaterial.difficulty === 'Medium' ? 'bg-yellow-500/20 text-yellow-400' :
              'bg-red-500/20 text-red-400'
            }`}>
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-lg transition-all duration-200 ${
                    ...
                    isSelected && !isCorrect ? 'bg-red-500/30 border border-red-500 text-white' :
                    isCorrect ? 'bg-[#00C9A7]/30 border border-[#00C9A7] text-white' :
