...
const DailyMissions = ({ userId, onMissionComplete, refreshTrigger }) => {
  ...
  const loadMissions = async () => {
    ...
    try {
      ...
      const points = missionData.reduce((sum, m) => sum + (m.completed ? m.points_earned : 0), 0);
      setStats({ completed, total, points, bonusEarned: completed === total && total > 0 });
    } catch (error) {
  ...
  const generateRandomMissions = async () => {
    try {
      ...
      const definitions = [
        ...
        { mission_type: 'riddle', target: 3, points: 75, name: 'Riddle Solver' },
        { mission_type: 'read', target: 1, points: 50, name: 'Knowledge Seeker' },
        { mission_type: 'score', target: 1, points: 100, name: 'Perfect Score' },
      ];
      ...
      if (error) throw error;
      setMissions(data || []);
      setStats({ completed: 0, total: data.length, points: 0, bonusEarned: false });
    } catch (error) {
  ...
  const handleMissionClick = (mission) => {
    ...
    navigate(paths[mission.mission_type] || '/categories');
    toast.info(`📝 Complete ${getMissionDescription(mission)} to earn points!`);
  };
  ...
  const handleMissionComplete = async (mission) => {
    ...
    if (!isActuallyCompleted) {
      toast.warning(`Complete ${getMissionDescription(mission)} to earn points!`);
      return;
    }
    ...
    try {
      ...
      const points = updatedMissions.reduce((sum, m) => sum + (m.completed ? m.points_earned : 0), 0);
      const bonusEarned = completed === total && total > 0;
      setStats({ completed, total, points, bonusEarned });

      toast.success(`✅ ${getMissionName(mission.mission_type)} complete! +${mission.points_earned} points`);
      if (onMissionComplete) onMissionComplete({ missionId: mission.id, completed, points });
      if (bonusEarned) await handleAllMissionsComplete();
    } catch (error) {
  ...
  const handleAllMissionsComplete = async () => {
    ...
    try {
      ...
      await supabase.from('users').update({ stats: updatedStats }).eq('id', userId);
      setStats((prev) => ({ ...prev, points: prev.points + bonusPoints, bonusEarned: true }));
  ...
  const getMissionIcon = (type) => {
    const icons = {
      ...
      riddle: <Brain className="w-4 h-4 text-primary" />,
      read: <BookOpen className="w-4 h-4 text-primary" />,
      score: <Target className="w-4 h-4 text-accent" />,
    };
  ...
  if (!missions || missions.length === 0) {
    return (
      <div className="glass-card p-6 text-center">
        ...
        <button onClick={refreshMissions} className="mt-2 text-xs text-primary hover:text-primary-light">
    ...
    <div className="glass-card p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400">{stats.completed}/{stats.total} complete</span>
          <button onClick={refreshMissions} disabled={refreshing} className="p-1 /15 rounded-lg">
            ...
            >
              <div className="flex items-center gap-3 flex-1">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (!isCompleted) handleMissionComplete(mission);
                  }}
                  disabled={isCompleted}
                  className={`flex-shrink-0 transition-all ${
                    isCompleted ? 'text-green-400 cursor-default' : 'text-gray-500 hover:text-primary cursor-pointer'
                  }`}
                >
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {!isCompleted && <ArrowRight className="w-4 h-4 text-gray-500" />}
                <span className="text-xs text-accent">+{mission.points_earned} pts</span>
      ...
      <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between text-xs text-gray-400">
        ...
        {stats.bonusEarned && (
          <div   className="text-accent font-medium flex items-center gap-1">
            <Award className="w-4 h-4" /> All Complete! 🎉
          </div>
        )}
      </div>
    </div>
